...
%   B...                blue channel of the image (changed by neutral white value)
    ...
    R = double(R) ./ asShotNeutral(1);
    G = double(G) ./ asShotNeutral(2);
    B = double(B) ./ asShotNeutral(3);
end
...
%   B...                blue channel of the image (without missing values)
    ...
    gruenFilter = [0.00 0.25 0.00; 0.25 1.00 0.25; 0.00 0.25 0.00];
    
    rotblauFilter = [0.25 0.50 0.25; 0.50 1.00 0.50; 0.25 0.50 0.25];
    
    
    R = imfilter(R, rotblauFilter);
    G = imfilter(G, gruenFilter);
    B = imfilter(B, rotblauFilter);
